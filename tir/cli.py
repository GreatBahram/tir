...
import tir
# deprecated
from tir import search, transform_date, transform_numerical_date, transform_number, 
...
op = OptionParser()
op.add_option('-s', '--solar', help='Does not show solar date', action='store_const', dest='solar', const=False, default=True)
op.add_option('-g', '--gregorian', help='Does not show gregorian date', action='store_const', dest='gregorian', const=False, default=True)
op.add_option('-c', '--calendar', help='Does not show calendar', action='store_const', dest='calendar', const=False, default=True)
op.add_option('-t', '--time', help='Does not show time', action='store_const', dest='time', const=False, default=True)
op.add_option('-C', '--color', help='Does not show colored text', action='store_const', dest='color', const=False, default=True)
op.add_option('-q', '--quote', help='Does not notify for quote', action='store_const', dest='quote', const=False, default=True)
op.add_option('-H', '--holidays', help='Does not notify for holidays', action='store_const', dest='holidays', const=False, default=True)
op.add_option('-a', '--about', help='shows program\'s description and exits', action='store_const', dest='about', const=True, default=False)
opts = op.parse_args()[0]

if opts.about:
    print('Command-line interface for http://time.ir website')
    sys.exit(0)
...
def find_date(rows, type_):
    row = search(rows, 'div', 'class', ('today-' + type_,))
...
class DrawCalendar:
    ...
    def _print(self, text, type_='n'):
        if self.theme and type_:
            if type_ == 'h':
...
class PrintTime:
    ...
    def _print(self, unit, type_=None):
        if self.theme and type_:
            if type_ == 'h':
...
class PrintDate:
    ...
    def _print(self, item, type_=None):
        if self.theme and type_:
            if type_ == 'y':
            ...
            elif type(type_) == int: # season number
                (start, stop) = self.theme.seasons[type_-1]
            else: # type_ == 'M':
...
class NotifyQuote(Notify):
    def __init__(self, quote):
        ...
        self.arguments = ['-t', str(time), 'time.ir - {}'.format(quote.author), quote.text]
...
class NotifyHolidays(Notify):
    def __init__(self, days):
        ...
        Notify.__init__(self)
        self.arguments = ['-t', '5000', 'time.ir', text]
...
def main():
    data = Request().get()
    transformers = {1: find_dates, 2: find_calendar, 3: find_quote}
    transformed = HTMLParser(data, transformers).parse()
    now = datetime.datetime.now()
    transformed[4] = Time(hour = transform_number(str(now.hour)), minute = transform_number(str(now.minute)), second = transform_number(str(now.second)))
    ...
    if opts.gregorian:
        ...
        if opts.color:
            date_theme = DateTheme(
                    year = ('\033[0;36m', '\033[0m'),
                    seasons = (('\033[1;31m', '\033[0m'), ('\033[1;31m', '\033[0m'), ('\033[1;33m', '\033[0m'), ('\033[1;36m', '\033[0m')),
                    month = ('\033[1;33m', '\033[0m'),
                    month_name= ('\033[1;35m', '\033[0m'),
                    weekday   = ('\033[1;34m', '\033[0m'),
                    day = ('\033[1;36m', '\033[0m'))
